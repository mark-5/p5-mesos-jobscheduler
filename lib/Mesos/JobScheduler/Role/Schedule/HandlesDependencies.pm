package Mesos::JobScheduler::Role::Schedule::HandlesDependencies;
use Moo::Role;
with "Mesos::JobScheduler::Role::Schedule";
...
after register => sub {
    my ($self, $job) = @_;
    $self->register_dependency($job) if $job->does("Mesos::JobScheduler::Role::Job::HasDependency");
};
