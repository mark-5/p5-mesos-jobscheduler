package Mesos::Framework::XUnit::Role::HasJobs;
use Types::Standard qw(HashRef ArrayRef Str);
use Time::HiRes qw(time);
use Mesos::Framework::XUnit::Utils qw(executor);
...
role {
    ...
    while (my ($name, $roles) = each %$jobs) {
        has $name => (
            is      => "ro",
            does    => "Mesos::Framework::Role::Job",
            builder => "_build_$name",
        );
        method "_build_$name" => sub {
            my $class = Moose::Meta::Class->create_anon_class(
                superclasses => [qw(Moose::Object)],
                roles        => [map {/::/ ? $_ : "Mesos::Framework::Role::Job::$_"} @$roles],
                cache        => 1,
            );
